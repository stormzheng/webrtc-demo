...
public class UserRegistry {
  ...
  public UserSession getBySession(SocketIOClient session) {
    return usersBySessionId.get(session.getSessionId().toString());
  }
  ...
  public UserSession removeBySession(SocketIOClient session) {
    final UserSession user = getBySession(session);
    if (user != null) {
      usersByName.remove(user.getName());
      usersBySessionId.remove(session.getSessionId().toString());
    }
    return user;
  }
